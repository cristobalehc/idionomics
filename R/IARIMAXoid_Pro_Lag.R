...
#Check weight RMA variable name.
  ...
  for(i in names) {
    ...
    #Create dataframe to correlate.
    cor_data <- data.frame(y_vect = y_vector, x_vect_lag = x_lag_vector ,x_vect = x_vector)


    options(warn = -1) #Supress spearman's warning about p-values with ties when used.
    correlation <- tryCatch(
      {
        ppcor::spcor(tidyr::drop_na(cor_data), method = correlation_method)
      },
      error = function(e) {
    ...
    model <- tryCatch(
      {
        forecast::auto.arima(y = y_vector, xreg = matrix(c(x_vector,x_lag_vector),
                                                         ncol=2, dimnames = list(NULL, c('xreg', 'xreg_lag'))), approximation = FALSE, stepwise = FALSE)
      },
      error = function(e) {
    ...
    #Fill XREG LAG parameters conditionally to their existence.
    if ('estimate_xreg_lag' %in% colnames(tidymodel)) {
      xreg_lag[[i]] <- tidymodel$estimate_xreg_lag
      stderr_xreg_lag[[i]] <- tidymodel$std.error_xreg_lag
    }     else {
      xreg_lag[[i]] <- NA
      stderr_xreg_lag[[i]] <- NA
    }
  ...
  results_df <- data.frame(
    ...
    xreg_lag = xreg_lag_vector,
    stderr_xreg_lag = stderr_xreg_lag_vector,
  ...
  if (hlm_compare == TRUE){
    ...
    if (weight_rma == TRUE & !is.null(weight_rma_var)) {
      ...
      meta_analysis <-
        tryCatch(
          {
            metafor::rma(yi = results_df$xreg_lag, sei = results_df$stderr_xreg_lag, weights = results_df$weight_variable)
          },
          error = function(e) {
      ...
      if (is.null(meta_analysis)){
        ...
        meta_analysis <-
          tryCatch(
            {
              metafor::rma(yi = results_df$xreg_lag, sei = results_df$stderr_xreg_lag, weights = results_df$weight_variable_n)
            },
            error = function(e) {
      ...
      if (is.null(meta_analysis)){
        ...
        meta_analysis <-
          tryCatch(
            {
              metafor::rma(yi = results_df$xreg_lag, sei = results_df$stderr_xreg_lag)
            },
            error = function(e) {
    ...
    if (is.null(hlm_model)){
      ...
      #If there are 4 rows in df_rand, and 4 fixed effects it includes time.
      if (ncol(df_rand) == 4 & length(hlm_model$coefficients$fixed) == 4){
      colnames(df_rand) <- c(id_var,'Intercept',"x_series", "x_series_lag")#Change names to legible ones.
      df_rand$random_slope <- df_rand[["x_series"]]+hlm_model$coefficients$fixed[3] #Create random slopes.
      df_rand$random_slope_lag <- df_rand[["x_series_lag"]]+hlm_model$coefficients$fixed[4] #Create random slopes.

      }
      #If there are 4 rows in df_rand, and 3 fixed effects it doesnt include time.
      if (ncol(df_rand) == 4 & length(hlm_model$coefficients$fixed) == 3){
        colnames(df_rand) <- c(id_var,'Intercept',"x_series", "x_series_lag")#Change names to legible ones.
        df_rand$random_slope <- df_rand[["x_series"]]+hlm_model$coefficients$fixed[2] #Create random slopes.
        df_rand$random_slope_lag <- df_rand[["x_series_lag"]]+hlm_model$coefficients$fixed[3] #Create random slopes.
      }

      #If there are 3 rows in df_rand, and 3 fixed effects it doesnt include rand for xreg, nor fixed time.
      if (ncol(df_rand) == 3 &  length(hlm_model$coefficients$fixed) == 3){
        colnames(df_rand) <- c(id_var,'Intercept', "x_series_lag")#Change names to legible ones.
        df_rand$random_slope_lag <- df_rand[["x_series_lag"]]+hlm_model$coefficients$fixed[3] #Create random slopes.
      }

      #If there are 3 rows in df_rand, and 3 fixed effects it doesnt include rand for xreg, nor fixed time or xreg.
      if (ncol(df_rand) == 3 &  length(hlm_model$coefficients$fixed) == 2){
        colnames(df_rand) <- c(id_var,'Intercept', "x_series_lag")#Change names to legible ones.
        df_rand$random_slope_lag <- df_rand[["x_series_lag"]]+hlm_model$coefficients$fixed[2] #Create random slopes.
      }

      #Values for comparison: For HLM will only extract lagged value.
      ...
      #Extract estimates and SE conditional to number of parameters.
      if (length(hlm_model$coefficients$fixed) == 4){
        hlmest <- hlm_model$coefficients$fixed[[4]] #Extract the fixed effect of x_series, the order depends on the formula.
        hlmse <- sqrt(hlm_model$varFix[16]) #Extract the se, for the fixed effect of x_series, the order depends on the formula.
      }
      if (length(hlm_model$coefficients$fixed) == 3){
      ...
      if (length(hlm_model$coefficients$fixed) == 2){
        hlmest <- hlm_model$coefficients$fixed[[2]] #Extract the fixed effect of x_series, the order depends on the formula.
        hlmse <- sqrt(hlm_model$varFix[4]) #Extract the se, for the fixed effect of x_series, the order depends on the formula.
      }

      iarimaxmean <- mean(results_df$xreg_lag, na.rm = TRUE) #Calculate mean of individual xregs.
      iarimaxvar <- stats::var(results_df$xreg_lag, na.rm = TRUE) #Calculate variance of individual xregs.
      ...
      hlmmean <- mean(df_rand$random_slope_lag, na.rm = TRUE) #Calculate mean of random slopes.
      hlmvar <- stats::var(df_rand$random_slope_lag, na.rm = TRUE) #Calculate variance of random slopes.

      #Opposite cases.
      num_oppcase_iarimax <- ifelse(meta_analysis$b[1] > 0,sum(results_df$xreg_lag <0, na.rm = TRUE),sum(results_df$xreg_lag > 0, na.rm = TRUE))
      num_oppcase_hlm <- ifelse(hlmest > 0,sum(df_rand$random_slope_lag <0, na.rm = TRUE),sum(df_rand$random_slope_lag > 0, na.rm = TRUE))
