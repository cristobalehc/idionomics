...
test_that("misspelled col triggers error naming the bad variable", {
  df <- make_pms_df()
  expect_error(
    pmstandardize(df, cols = "not_a_col", id_var = "id"),
    regexp = "not_a_col"
  )
})
...
test_that("misspelled id_var triggers error naming the bad variable", {
  df <- make_pms_df()
  expect_error(
    pmstandardize(df, cols = "x", id_var = "not_an_id"),
    regexp = "not_an_id"
  )
})
...
test_that("error message includes 'Cannot find required variables'", {
  df <- make_pms_df()
  expect_error(
    pmstandardize(df, cols = "oops", id_var = "id"),
...
test_that("id_var as a vector triggers error", {
  df <- make_pms_df()
  expect_error(
    pmstandardize(df, cols = "x", id_var = c("id", "id")),
    regexp = "id_var"
  )
})
...
test_that("empty cols vector triggers error", {
  df <- make_pms_df()
  expect_error(
    pmstandardize(df, cols = character(0), id_var = "id"),
    regexp = "cols"
  )
})
...
test_that("append = TRUE preserves all original columns", {
  df     <- make_pms_df()
  result <- pmstandardize(df, cols = "x", id_var = "id", append = TRUE)
...
test_that("append = FALSE returns only id_var and _psd columns", {
  df     <- make_pms_df()
  result <- pmstandardize(df, cols = "x", id_var = "id", append = FALSE)
...
test_that("multiple cols produce a _psd column for each", {
  df     <- make_pms_df()
  result <- pmstandardize(df, cols = c("x", "y"), id_var = "id")
  expect_true("x_psd" %in% names(result))
...
test_that("verbose = TRUE emits messages", {
  ...
  suppressMessages(
    expect_message(
      pmstandardize(df, cols = "x", id_var = "id", verbose = TRUE)
    )
  )
})
...
test_that("verbose = FALSE emits no messages", {
  df <- make_pms_df()
  expect_no_message(
    pmstandardize(df, cols = "x", id_var = "id", verbose = FALSE)
  )
})
